...
class Chart extends StatelessWidget {
  ...
  Chart({Key? key, required this.isExpense, required this.myTransactions}) : super(key: key);
  ...
  List<Map<String, dynamic>> get groupedExpenseTx {
    return List.generate(7, (i) {
      ...
      print('amountntttt eeeeeee $totalSum');
      return {
  ...
  List<Bar> _displayChart() {
    if (isExpense) {
      if (myTransactions.any((element) => element.type == 'Expense')) {
        ...
        return weekDays
            .map((day) => Bar(isExpense: true, label: day, totalAmount: 0, percentage: 0))
            .toList();
      }
    } else {
